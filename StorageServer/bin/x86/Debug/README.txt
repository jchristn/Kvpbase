Thank you for choosing kvpbase!  Run storage-server.exe and we will create the 
...
- Linux   : $ sudo mono --server storage-server.exe
- Mac     : $ sudo mono --server storage-server.exe
...
the system.config file to change the admin API key, set the port number, and 
enable SSL.
