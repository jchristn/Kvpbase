    ...
    public class ReplicationHandler
    {
        ...
        public bool ContainerWrite(Obj currObj, out List<Node> nodes)
        {
            ...
            switch (currObj.ReplicationMode)
            {
                ...
                default:
                    Logging.Log(LoggingModule.Severity.Warn, "ContainerWrite unknown replication_mode in path object: " + currObj.ReplicationMode);
        ...
        public bool ObjectRename(RenameRequest currRename, Obj currObj)
        {
            ...
            if (currObj.Replicas != null)
            {
                foreach (Node curr_node in currObj.Replicas)
                {
                    switch (currObj.ReplicationMode)
                    {
                        case "none":
                        ...
                        case "async":
                            ...
                            Task.Run(() => ObjectRenameReplicaAsync(currRename, curr_node));
                            break;

                        case "sync":
                            Logging.Log(LoggingModule.Severity.Debug, "ObjectRename sync replication mode specified");
                            if (!ObjectRenameReplica(currRename, curr_node))
                            {
                                Logging.Log(LoggingModule.Severity.Warn, "ObjectRename failed replication to node ID " + curr_node.NodeId + " " + curr_node.Name + " for rename operation");
                                Task.Run(() => ObjectRenameReplicaAsync(currRename, curr_node));
                            }
                            else
                            {
                                successfulRenames.Add(curr_node);
                            }
                            break;

                        default:
                            Logging.Log(LoggingModule.Severity.Warn, "ObjectRename unknown replication_mode in obj: " + currObj.ReplicationMode);
                            Common.ExitApplication("ObjectRename", "Unknown replication mode", -1);
                            return false;
                    }
                }
        ...
        public bool ObjectWrite(Obj currObj, out List<Node> nodes)
        {
            ...
            if (currObj == null)
            {
                Logging.Log(LoggingModule.Severity.Warn, "ObjectWrite null value detected for obj");
                return false;
            }
            ...
            switch (currObj.ReplicationMode)
            {
                ...
                default:
                    Logging.Log(LoggingModule.Severity.Warn, "ObjectWrite unknown replication_mode in obj: " + currObj.ReplicationMode);
            ...
            if (CurrentTopology.Replicas != null)
            {
                foreach (Node curr_node in CurrentTopology.Replicas)
                {
                    if (!ObjectWriteReplica(currObj, curr_node))
                    {
                        Logging.Log(LoggingModule.Severity.Warn, "ObjectWrite failed replication to node_id " + curr_node.NodeId + " " + curr_node.Name + " for key " + currObj.Key);
                        Task.Run(() =>
                        {
                            ReplicationHandler rh = new ReplicationHandler(CurrentSettings, CurrentTopology, CurrentNode, Users, Logging);
                    ...
                    {
                        successfulWrites.Add(curr_node);
                    }
                }
            }
        ...
        public HttpResponse ServerObjectReceive(RequestMetadata md)
        {
            ...
            {
                Logging.Log(LoggingModule.Severity.Warn, "ServerObjectReceive unable to build disk_path from request");
                return new HttpResponse(md.CurrentHttpRequest, false, 500, null, "application/json",
        ...
        public HttpResponse ServerContainerReceive(RequestMetadata md)
        {
            ...
            if (String.IsNullOrEmpty(req.DiskPath))
            {
                Logging.Log(LoggingModule.Severity.Warn, "ServerContainerReceive unable to build disk_path from request");
                return new HttpResponse(md.CurrentHttpRequest, false, 500, null, "application/json",
        ...
        private bool ContainerRenameReplica(RenameRequest currRename, Node currNode)
        {
            ...
            if (currNode == null)
            {
                Logging.Log(LoggingModule.Severity.Warn, "ContainerWriteReplica null value for curr_node");
                return false;
            }
        ...
        {
            Logging.Log(LoggingModule.Severity.Debug, "ObjectMoveReplicaAsync entering");
            ...
            if (currNode == null)
            {
                Logging.Log(LoggingModule.Severity.Warn, "ObjectMoveReplicaAsync null value for curr_node");
                return;
            }
        ...
        {
            Logging.Log(LoggingModule.Severity.Debug, "ObjectMoveReplica entering");
            ...
            if (currNode == null)
            {
                Logging.Log(LoggingModule.Severity.Warn, "ObjectMoveReplica null value for curr_node");
                return false;
            }
        ...
        {
            Logging.Log(LoggingModule.Severity.Debug, "ObjectRenameReplicaAsync entering");
        ...
        {
            Logging.Log(LoggingModule.Severity.Debug, "ObjectRenameReplica entering");
            ...
            if (currNode == null)
            {
                Logging.Log(LoggingModule.Severity.Warn, "ObjectRenameReplica null value for curr_node");
                return false;
            }
        ...
        private void ObjectWriteReplicaAsync(Obj currObj)
        {
            ...
            foreach (Node currNode in currObj.Replicas)
            {
                ...
                {
                    Logging.Log(LoggingModule.Severity.Warn, "ObjectWriteReplicaAsync unable to replicate object " + currObj.Key + " to node_id " + currNode.NodeId + " " + currNode.Name);
                    SendReplica(currNode, "POST /admin/replication/object", Common.SerializeJson(currObj));
                }
                else
                {
                    Logging.Log(LoggingModule.Severity.Debug, "ObjectWriteReplicaAsync successfully replicated object " + currObj.Key + " to node_id " + currNode.NodeId + " " + currNode.Name);
                }
            }
        ...
        private bool ObjectWriteReplica(Obj currObj, Node currNode)
        {
            ...
            if (currNode == null)
            {
                Logging.Log(LoggingModule.Severity.Warn, "ObjectWriteReplica null value for curr_node");
                return false;
            }
