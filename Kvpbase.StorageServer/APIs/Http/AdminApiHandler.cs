...
namespace Kvpbase.StorageServer
{
    public partial class Program
    {
        static async Task AdminApiHandler(RequestMetadata md)
        {
            ...
            {
                case HttpMethod.GET:  
                    if (md.Http.Request.RawUrlEntries[1].Equals("disk"))
                    {
                        await HttpGetDisks(md);
                        return;
                    }
                    else if (md.Http.Request.RawUrlEntries[1].Equals("config"))
                    {
            ...
            _Logging.Warn(header + "AdminApiHandler unknown URL " + md.Http.Request.RawUrlWithoutQuery);
